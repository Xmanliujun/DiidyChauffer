...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setDidTimeOutSelector:@selector(requestTimeOutCallBack:)];
	[self setURL:newURL];
...
//		[self failWithError:ASIRequestTimedOutError];
		if (delegate && [delegate respondsToSelector:didTimeOutSelector]) {
	...
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
                ...
                [self setDefaultUserAgentString:[NSString stringWithFormat:@"%@ %@ (%@; %@ %@; %@)",@"lehecai", appVersion, /*[LTools getObjectFromSystemKey:@"productType"]*/nil, OSName, OSVersion, locale]];
			#else
                ...
                [self setDefaultUserAgentString:[NSString stringWithFormat:@"%@ %@ (%@; %@ %@; %@)",@"lehecai", appVersion, [LTools getObjectFromSystemKey:@"productType"], OSName, OSVersion, locale]];
			#endif

			// Takes the form "My Application 1.0 (Macintosh; Mac OS X 10.5.7; en_GB)"
			
//            NSLog(@"++++++++++++++++%@",[NSString stringWithFormat:@"%@ %@ (%@; %@ %@; %@)",@"lehecai", appVersion, [LTools getObjectFromSystemKey:@"productType"], OSName, OSVersion, locale]);
		}
