...
- (NSString*)unEncodeURL:(NSString *)string{
    if (string) {
        return string;
    }
    return @"";
...
- (void)buildURLEncodedPostBody
{
	...
	for (NSDictionary *val in [self postData]) {
#ifdef DO_NOT_ENCODURL
        NSString *data = [NSString stringWithFormat:@"%@=%@%@", [self unEncodeURL:[val objectForKey:@"key"]], [self unEncodeURL:[val objectForKey:@"value"]],(i<count ?  @"&" : @"")];
#else
